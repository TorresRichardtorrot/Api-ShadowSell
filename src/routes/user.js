...
router.get("/users/:email/:clave", async (req, res) => {
  ...
  try {
    const usuario = await userSchema.findOne({ email,clave });
      // console.log(usuario)
    if (!usuario) {
...
router.post('/recuperarP', async (req,res) => {
  ...
  const resetURL = `http://localhost:9000/shadowsell/restablecer/${usuario.Token}`
  ...
  try {
    await fetch(`http://localhost:9000/shadowsell/user/enviar`, {
      method: 'POST',
      headers: {
...
res.redirect('http://localhost:3000/login/restablecerP.html')
